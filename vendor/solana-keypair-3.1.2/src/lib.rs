...
impl Keypair {
    ...
    pub fn try_from_base58_string(s: &str) -> Result<Self, SignatureError> {
        ...
        five8::decode_64(s, &mut buf).map_err(|err| {
            let message = match err {
                five8::DecodeError::InvalidChar(c) => {
                    format!("Illegal base58 char number: {c}")
                }
                five8::DecodeError::TooLong => String::from("Base58 string too long"),
                five8::DecodeError::TooShort => String::from("Base58 string too short"),
                five8::DecodeError::LargestTermTooHigh => {
                    String::from("Largest term greater than 2^32")
                }
                five8::DecodeError::OutputTooLong => {
                    String::from("Decoded output has too many bytes")
                }
            };
            SignatureError::from_source(message)
        })?;
        Self::try_from(&buf[..])
    }
